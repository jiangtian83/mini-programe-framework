...
class Console 
{
	protected $config = array( "trace_tabs" => array( "base" => "基本", "file" => "文件", "info" => "流程", "notice|error" => "错误", "sql" => "SQL", "debug|log" => "调试" ) );
	...
	protected function console($type, $msg) 
	{
		...
		foreach( (array) $msg as $key => $m ) 
		{
			switch( $type ) 
			{
				case "调试": $var_type = gettype($m);
				if( in_array($var_type, array( "array", "string" )) ) 
				{
				...
				break;
				case "错误": $msg = str_replace("\n", "\\n", json_encode($m));
				$style = "color:#F4006B;font-size:14px;";
				$line[] = "console.error(\"%c" . $msg . "\", \"" . $style . "\");";
				break;
				case "sql": $msg = str_replace("\n", "\\n", $m);
				$style = "color:#009bb4;";
				$line[] = "console.log(\"%c" . $msg . "\", \"" . $style . "\");";
				break;
				default: $m = (is_string($key) ? $key . " " . $m : ($key + 1) . " " . $m);
				$msg = json_encode($m);
