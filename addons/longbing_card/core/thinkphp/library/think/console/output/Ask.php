...
class Ask 
{
	...
	public function __construct(\think\console\Input $input, \think\console\Output $output, Question $question) 
	{
		$this->input = $input;
		$this->output = $output;
	...
	protected function doAsk() 
	{
		...
		$ret = (0 < strlen($ret) ? $ret : $this->question->getDefault());
	...
	protected function writePrompt() 
	{
		...
		switch( true ) 
		{
			case null === $default: $text = sprintf(" <info>%s</info>:", $text);
			break;
			case $this->question instanceof question\Confirmation: $text = sprintf(" <info>%s (yes/no)</info> [<comment>%s</comment>]:", $text, ($default ? "yes" : "no"));
			break;
			case $this->question instanceof question\Choice && $this->question->isMultiselect(): $choices = $this->question->getChoices();
			...
			break;
			case $this->question instanceof question\Choice: $choices = $this->question->getChoices();
			$text = sprintf(" <info>%s</info> [<comment>%s</comment>]:", $text, $choices[$default]);
			break;
			default: $text = sprintf(" <info>%s</info> [<comment>%s</comment>]:", $text, $default);
		}
		$this->output->writeln($text);
		if( $this->question instanceof question\Choice ) 
		{
