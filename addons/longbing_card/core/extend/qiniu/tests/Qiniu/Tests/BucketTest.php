...
class BucketTest extends \PHPUnit_Framework_TestCase 
{
	...
	protected $dummyBucketManager = NULL;
	...
	protected function setUp() 
	{
		...
		$this->key2 = $key2;
		global $testAuth;
		$this->bucketManager = new \Qiniu\Storage\BucketManager($testAuth);
		global $dummyAuth;
		$this->dummyBucketManager = new \Qiniu\Storage\BucketManager($dummyAuth);
	}
	...
	public function testCopy() 
	{
		...
		$error = $this->bucketManager->copy($this->bucketName, $this->key, $this->bucketName, $key);
		$this->assertNull($error);
		$error = $this->bucketManager->copy($this->bucketName, $this->key2, $this->bucketName, $key, true);
		$this->assertNull($error);
	...
	{
		$error = $this->bucketManager->changeMime($this->bucketName, "php-sdk.html", "text/html");
		$this->assertNull($error);
	}
	public function testPrefetch() 
	{
		$error = $this->bucketManager->prefetch($this->bucketName, "php-sdk.html");
		$this->assertNull($error);
	}
	public function testFetch() 
	{
		list($ret, $error) = $this->bucketManager->fetch("http://developer.qiniu.com/docs/v6/sdk/php-sdk.html", $this->bucketName, "fetch.html");
		$this->assertArrayHasKey("hash", $ret);
		$this->assertNull($error);
		list($ret, $error) = $this->bucketManager->fetch("http://developer.qiniu.com/docs/v6/sdk/php-sdk.html", $this->bucketName, "");
	...
	{
		$key = "copyto" . rand();
		$ops = \Qiniu\Storage\BucketManager::buildBatchCopy($this->bucketName, array( $this->key => $key ), $this->bucketName, true);
		list($ret, $error) = $this->bucketManager->batch($ops);
		$this->assertEquals(200, $ret[0]["code"]);
		$ops = \Qiniu\Storage\BucketManager::buildBatchDelete($this->bucketName, array( $key ));
		list($ret, $error) = $this->bucketManager->batch($ops);
		$this->assertEquals(200, $ret[0]["code"]);
	}
	public function testBatchMove() 
	{
		...
		$this->bucketManager->copy($this->bucketName, $this->key, $this->bucketName, $key);
		$key2 = $key . "to";
		$ops = \Qiniu\Storage\BucketManager::buildBatchMove($this->bucketName, array( $key => $key2 ), $this->bucketName, true);
		list($ret, $error) = $this->bucketManager->batch($ops);
		$this->assertEquals(200, $ret[0]["code"]);
		$error = $this->bucketManager->delete($this->bucketName, $key2);
		$this->assertNull($error);
	...
	public function testBatchRename() 
	{
		...
		$this->bucketManager->copy($this->bucketName, $this->key, $this->bucketName, $key);
		$key2 = $key . "to";
		$ops = \Qiniu\Storage\BucketManager::buildBatchRename($this->bucketName, array( $key => $key2 ), true);
		list($ret, $error) = $this->bucketManager->batch($ops);
		$this->assertEquals(200, $ret[0]["code"]);
		$error = $this->bucketManager->delete($this->bucketName, $key2);
		$this->assertNull($error);
	...
	{
		$ops = \Qiniu\Storage\BucketManager::buildBatchStat($this->bucketName, array( "php-sdk.html" ));
		list($ret, $error) = $this->bucketManager->batch($ops);
		$this->assertEquals(200, $ret[0]["code"]);
	}
