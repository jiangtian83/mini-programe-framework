...
class BatchTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$batch = new \Guzzle\Batch\Batch($this->getMockTransfer(), $this->getMockDivisor());
	...
	public function testFlushReturnsItems() 
	{
		...
		$batch = new \Guzzle\Batch\Batch($transfer, $divisor);
	...
	public function testThrowsExceptionContainingTheFailedBatch() 
	{
		...
		$transfer = $this->getMockTransfer();
		$transfer->expects($this->exactly(2))->method("transfer")->will($this->returnCallback(function() use (&$called, $originalException) 
		{
			if( ++$called == 2 ) 
			{
		...
		$divisor->expects($this->once())->method("createBatches")->will($this->returnCallback(function($batch) use ($queue) 
		{
