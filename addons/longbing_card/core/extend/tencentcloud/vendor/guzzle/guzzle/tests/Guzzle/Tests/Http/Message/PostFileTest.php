...
class PostFileTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$file = new \Guzzle\Http\Message\PostFile("foo", __FILE__, "x-foo", "boo");
	...
	{
		$file = new \Guzzle\Http\Message\PostFile("foo", "@" . __FILE__);
		$this->assertEquals(__FILE__, $file->getFilename());
	}
	public function testEnsuresFileIsReadable() 
	{
		$file = new \Guzzle\Http\Message\PostFile("foo", "/foo/baz/bar");
	}
	...
	{
		$file = new \Guzzle\Http\Message\PostFile("foo", __FILE__, null, "NewPostFileTest.php");
		if( version_compare(phpversion(), "5.5.0", "<") ) 
		{
