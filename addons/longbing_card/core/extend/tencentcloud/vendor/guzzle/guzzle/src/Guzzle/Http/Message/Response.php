...
class Response extends AbstractMessage implements \Serializable 
{
	...
	protected $effectiveUrl = NULL;
	protected static $cacheResponseCodes = array( 200, 203, 206, 300, 301, 410 );
	public static function fromMessage($message) 
	{
		$data = \Guzzle\Parser\ParserRegistry::getInstance()->getParser("message")->parseResponse($message);
		if( !$data ) 
		{
			return false;
		}
		...
		if( 0 < strlen($data["body"]) && $contentLength != $actualLength ) 
		{
	...
	public function __construct($statusCode, $headers = NULL, $body = NULL) 
	{
		...
		$this->body = \Guzzle\Http\EntityBody::factory(($body !== null ? $body : ""));
		if( $headers ) 
		{
			if( is_array($headers) ) 
			{
			...
			{
				if( $headers instanceof \Guzzle\Common\ToArrayInterface ) 
				{
					$this->setHeaders($headers->toArray());
				}
				else 
				{
					throw new \Guzzle\Http\Exception\BadResponseException("Invalid headers argument received");
				}
			}
		}
	}
	...
	{
		return 300 <= $this->statusCode && $this->statusCode < 400;
	}
	...
	{
		return 200 <= $this->statusCode && $this->statusCode < 300 || $this->statusCode == 304;
	}
	public function canCache() 
	{
		...
		if( (!$this->getBody()->isReadable() || !$this->getBody()->isSeekable()) && (0 < $this->getContentLength() || $this->getTransferEncoding() == "chunked") ) 
		{
			return false;
		}
	...
	public function isFresh() 
	{
		...
		return ($fresh === null ? null : 0 <= $fresh);
	}
	...
	public function json() 
	{
		...
		{
			throw new \Guzzle\Common\Exception\RuntimeException("Unable to parse response body into JSON: " . json_last_error());
		}
	...
	public function xml() 
	{
		...
		if( $errorMessage ) 
		{
			throw new \Guzzle\Common\Exception\RuntimeException("Unable to parse response body into XML: " . $errorMessage);
		}
		return $xml;
	}
	public function getRedirectCount() 
	{
		return (int) $this->params->get(\Guzzle\Http\RedirectPlugin::REDIRECT_COUNT);
	}
	...
	{
		\Guzzle\Common\Version::warn("Guzzle\\Http\\Message\\Response::getPreviousResponse" . " is deprecated. Use the HistoryPlugin.");
		return null;
	}
	public function setRequest($request) 
	{
		\Guzzle\Common\Version::warn("Guzzle\\Http\\Message\\Response::setRequest" . " is deprecated");
		return $this;
	}
	public function getRequest() 
	{
		\Guzzle\Common\Version::warn("Guzzle\\Http\\Message\\Response::getRequest" . " is deprecated");
		return null;
	}
}
?>