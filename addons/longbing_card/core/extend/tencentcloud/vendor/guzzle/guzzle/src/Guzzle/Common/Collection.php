...
class Collection implements \ArrayAccess, \IteratorAggregate, \Countable, ToArrayInterface 
{
	...
	public static function fromConfig(array $config = array( ), array $defaults = array( ), array $required = array( )) 
	{
		...
		{
			throw new Exception\InvalidArgumentException("Config is missing the following keys: " . implode(", ", $missing));
		}
	...
	public function setPath($path, $value) 
	{
		...
		while( null !== ($key = array_shift($queue)) ) 
		{
			if( !is_array($current) ) 
			{
				throw new Exception\RuntimeException("Trying to setPath " . $path . ", but " . $key . " is set and is not an array");
			}
			if( !$queue ) 
			{
				$current[$key] = $value;
		...
		while( null !== ($part = array_shift($path)) ) 
		{
			...
			else 
			{
				...
				foreach( $data as $value ) 
				{
					...
					{
						if( null !== ($test = $this->getPath($path, $separator, $value)) ) 
						{
	...
	{
		Version::warn("Guzzle\\Common\\Collection::inject" . " is deprecated");
		$replace = array( );
