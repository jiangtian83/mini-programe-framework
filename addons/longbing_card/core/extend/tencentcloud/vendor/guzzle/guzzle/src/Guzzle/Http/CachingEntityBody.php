...
class CachingEntityBody extends AbstractEntityBodyDecorator 
{
	...
	public function seek($offset, $whence = SEEK_SET) 
	{
		...
		else 
		{
			...
			{
				throw new \Guzzle\Common\Exception\RuntimeException("Guzzle\\Http\\CachingEntityBody" . " supports only SEEK_SET and SEEK_CUR seek operations");
			}
		}
		if( $this->body->getSize() < $byte ) 
		{
			throw new \Guzzle\Common\Exception\RuntimeException("Cannot seek to byte " . $byte . " when the buffered stream only contains " . $this->body->getSize() . " bytes");
		}
	...
	{
		throw new \Guzzle\Common\Exception\RuntimeException("Guzzle\\Http\\CachingEntityBody" . " does not support custom stream rewind functions");
	}
