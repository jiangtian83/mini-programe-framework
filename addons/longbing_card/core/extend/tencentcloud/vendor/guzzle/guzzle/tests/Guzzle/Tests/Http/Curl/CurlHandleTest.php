...
{
	public $requestHandle = NULL;
	protected function updateForHandle(\Guzzle\Http\Message\RequestInterface $request) 
	{
		$that = $this;
		$request->getEventDispatcher()->addListener("request.sent", function(\Guzzle\Common\Event $e) use ($that) 
		{
	...
	{
		$h = new \Guzzle\Http\Curl\CurlHandle(false, array( ));
	}
	public function testConstructorExpectsProperOptions() 
	{
		...
		try 
		{
			$ha = new \Guzzle\Http\Curl\CurlHandle($h, false);
			$this->fail("Expected InvalidArgumentException");
		}
		catch( \InvalidArgumentException $e ) 
		{
		}
		$ha = new \Guzzle\Http\Curl\CurlHandle($h, array( CURLOPT_URL => $this->getServer()->getUrl() ));
		$this->assertEquals($this->getServer()->getUrl(), $ha->getOptions()->get(CURLOPT_URL));
		$ha = new \Guzzle\Http\Curl\CurlHandle($h, new \Guzzle\Common\Collection(array( CURLOPT_URL => $this->getServer()->getUrl() )));
		$this->assertEquals($this->getServer()->getUrl(), $ha->getOptions()->get(CURLOPT_URL));
	}
	public function testConstructorInitializesObject() 
	{
		$handle = curl_init($this->getServer()->getUrl());
		$h = new \Guzzle\Http\Curl\CurlHandle($handle, array( CURLOPT_URL => $this->getServer()->getUrl() ));
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://test.com");
		$request->getCurlOptions()->set("debug", true);
		$h = \Guzzle\Http\Curl\CurlHandle::factory($request);
	...
	{
		$h = new \Guzzle\Http\Curl\CurlHandle(curl_init("http://test.com"), array( CURLOPT_URL => "http://test.com" ));
	...
	public function testAccountsForMissingStdErr() 
	{
		...
		$h = new \Guzzle\Http\Curl\CurlHandle($handle, array( CURLOPT_URL => "http://www.test.com/" ));
		$this->assertNull($h->getStderr(false));
	}
	public function testDeterminesIfResourceIsAvailable() 
	{
		$handle = curl_init($this->getServer()->getUrl());
		$h = new \Guzzle\Http\Curl\CurlHandle($handle, array( ));
	...
	public function testWrapsErrorsAndInfo() 
	{
		...
		$settings = array( CURLOPT_PORT => 123, CURLOPT_CONNECTTIMEOUT_MS => 1, CURLOPT_TIMEOUT_MS => 1 );
		$handle = curl_init($this->getServer()->getUrl());
		curl_setopt_array($handle, $settings);
		$h = new \Guzzle\Http\Curl\CurlHandle($handle, $settings);
		@curl_exec($handle);
		$errors = array( "couldn't connect to host", "timeout was reached", "connection time-out", "connect() timed out!", "failed connect to 127.0.0.1:123; connection refused", "failed to connect to 127.0.0.1 port 123: connection refused" );
		$this->assertTrue(in_array(strtolower($h->getError()), $errors), $h->getError() . " was not the error");
		$this->assertTrue(0 < $h->getErrorNo());
	...
	{
		$handle = curl_init($this->getServer()->getUrl());
		$h = new \Guzzle\Http\Curl\CurlHandle($handle, array( CURLOPT_AUTOREFERER => true, CURLOPT_BUFFERSIZE => 1024 ));
	...
	public function testFactoryCreatesCurlBasedOnRequest($method, $url, $headers, $body, $options, $expectedHeaders = NULL) 
	{
		...
		$handle = \Guzzle\Http\Curl\CurlHandle::factory($curlTest);
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://127.0.0.1:8124/");
	...
	public function testUploadsPutData() 
	{
		...
		$request = $client->put("/");
		$request->getCurlOptions()->set("debug", true);
		$request->setBody(\Guzzle\Http\EntityBody::factory("test"), "text/plain", false);
	...
	public function testUploadsPutDataUsingChunkedEncodingWhenLengthCannotBeDetermined() 
	{
		...
		$client = new \Guzzle\Http\Client($this->getServer()->getUrl());
		$request = $client->put("/");
		$request->setBody(\Guzzle\Http\EntityBody::factory(fopen($this->getServer()->getUrl(), "r")), "text/plain");
		$request->send();
		$r = $this->getServer()->getReceivedRequests(true);
	...
	public function testSendsPostRequestsWithFields() 
	{
		$this->getServer()->flush();
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\nhi");
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", $this->getServer()->getUrl());
		$request->getCurlOptions()->set("debug", true);
		$request->setClient(new \Guzzle\Http\Client());
		$request->addPostFields(array( "a" => "b", "c" => "ay! ~This is a test, isn't it?" ));
		$request->send();
		$r = $this->getServer()->getReceivedRequests(true);
	...
	public function testSendsPostRequestsWithFiles() 
	{
		...
		$request->addPostFields(array( "bar" => "baz", "arr" => array( "a" => 1, "b" => 2 ) ));
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", $this->getServer()->getUrl());
		$request->setClient(new \Guzzle\Http\Client("http://www.example.com"));
	...
	{
		$actual = \Guzzle\Http\Curl\CurlHandle::parseCurlConfig($options);
		$this->assertEquals($expected, $actual);
	}
	public function provideCurlConfig() 
	{
		return array( array( array( "CURLOPT_PORT" => 10, "CURLOPT_TIMEOUT" => 99 ), array( CURLOPT_PORT => 10, CURLOPT_TIMEOUT => 99 ) ), array( array( "debug" => true ), array( "debug" => true ) ), array( array( "debug" => "CURLPROXY_HTTP" ), array( "debug" => CURLPROXY_HTTP ) ) );
	}
	...
	public function testSetsCurloptEncodingWhenAcceptEncodingHeaderIsSet() 
	{
		...
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 4\r\n\r\ndata");
		$client = new \Guzzle\Http\Client($this->getServer()->getUrl());
		$request = $client->get("/", array( "Accept" => "application/json", "Accept-Encoding" => "gzip, deflate" ));
		$this->updateForHandle($request);
		$request->send();
		$options = $this->requestHandle->getOptions()->getAll();
	...
	public function testSendsPostFieldsForNonPostRequests() 
	{
		...
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\n\r\nContent-Length: 0\r\n\r\n");
		$client = new \Guzzle\Http\Client();
		$request = $client->put($this->getServer()->getUrl(), null, array( "foo" => "baz", "baz" => "bar" ));
		$request->send();
		$requests = $this->getServer()->getReceivedRequests(true);
	...
	public function testSendsPostFilesForNonPostRequests() 
	{
		...
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\n\r\nContent-Length: 0\r\n\r\n");
		$client = new \Guzzle\Http\Client();
		$request = $client->put($this->getServer()->getUrl(), null, array( "foo" => "@" . __FILE__ ));
		$request->send();
		$requests = $this->getServer()->getReceivedRequests(true);
		$this->assertEquals("PUT", $requests[0]->getMethod());
