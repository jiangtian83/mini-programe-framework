...
class CachingEntityBodyTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$this->decorated = \Guzzle\Http\EntityBody::factory("testing");
		$this->body = new \Guzzle\Http\CachingEntityBody($this->decorated);
	}
	public function testUsesRemoteSizeIfPossible() 
	{
		$body = \Guzzle\Http\EntityBody::factory("test");
		$caching = new \Guzzle\Http\CachingEntityBody($body);
	...
	{
		$a = \Guzzle\Http\EntityBody::factory("foo");
		$d = $this->getMockBuilder("Guzzle\\Http\\CachingEntityBody")->setMethods(array( "seek" ))->setConstructorArgs(array( $a ))->getMock();
		$d->expects($this->once())->method("seek")->with(0)->will($this->returnValue(true));
		$d->rewind();
	}
	...
	{
		$decorated = \Guzzle\Http\EntityBody::factory(implode("\n", array_map(function($n) 
		{
			return str_pad($n, 4, "0", STR_PAD_LEFT);
		}
		, range(0, 25))));
		$body = new \Guzzle\Http\CachingEntityBody($decorated);
	...
	public function testWrapsContentType() 
	{
		...
		$a->expects($this->once())->method("getContentType")->will($this->returnValue("foo"));
		$d = new \Guzzle\Http\CachingEntityBody($a);
	...
	{
		$a = $this->getMockBuilder("Guzzle\\Http\\EntityBody")->setMethods(array( "getCustomData", "setCustomData" ))->setConstructorArgs(array( fopen(__FILE__, "r") ))->getMock();
		$a->expects($this->exactly(1))->method("getCustomData")->with("foo")->will($this->returnValue("bar"));
		$a->expects($this->exactly(1))->method("setCustomData")->with("foo", "bar")->will($this->returnSelf());
		$d = new \Guzzle\Http\CachingEntityBody($a);
	...
	{
		$s = fopen("php://temp", "r");
		$a = \Guzzle\Http\EntityBody::factory($s);
		$d = new \Guzzle\Http\CachingEntityBody($a);
		$d->close();
