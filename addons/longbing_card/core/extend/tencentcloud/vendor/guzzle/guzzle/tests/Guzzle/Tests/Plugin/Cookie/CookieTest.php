...
class CookieTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$cookie = new \Guzzle\Plugin\Cookie\Cookie(array( "expires" => "November 20, 1984" ));
		$this->assertTrue(is_numeric($cookie->getExpires()));
	}
	public function testAddsExpiresBasedOnMaxAge() 
	{
		$t = time();
		$cookie = new \Guzzle\Plugin\Cookie\Cookie(array( "max_age" => 100 ));
	...
	public function testHoldsValues() 
	{
		...
		$cookie = new \Guzzle\Plugin\Cookie\Cookie($data);
	...
	{
		$c = new \Guzzle\Plugin\Cookie\Cookie();
		$c->setExpires(10);
	...
	{
		return array( array( "foo", "baz", "bar", true ), array( "0", "0", "0", true ), array( "", "baz", "bar", "The cookie name must not be empty" ), array( "foo", "", "bar", "The cookie value must not be empty" ), array( "foo", "baz", "", "The cookie domain must not be empty" ), array( "foo\\", "baz", "0", "The cookie name must not contain invalid characters: foo\\" ) );
	}
	public function testValidatesCookies($name, $value, $domain, $result) 
	{
		$cookie = new \Guzzle\Plugin\Cookie\Cookie(array( "name" => $name, "value" => $value, "domain" => $domain ));
