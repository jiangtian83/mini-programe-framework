...
class HttpRequestFactoryTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$factory = \Guzzle\Http\Message\RequestFactory::getInstance();
		$this->assertSame($factory, \Guzzle\Http\Message\RequestFactory::getInstance());
	}
	public function testCreatesNewGetRequests() 
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://www.google.com/");
		...
		$b = \Guzzle\Http\EntityBody::factory();
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", $this->getServer()->getUrl(), null, $b);
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/path?q=1&v=2", null, "Data");
		...
		unset($request);
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/path?q=1&v=2", null, \Guzzle\Http\EntityBody::factory("Data"));
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("Data", (string) $request->getBody());
		$resource = fopen("php://temp", "w+");
		fwrite($resource, "Data");
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/path?q=1&v=2", null, $resource);
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("Data", (string) $request->getBody());
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/path?q=1&v=2", null, \Guzzle\Http\Url::factory("http://www.example.com/"));
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("http://www.example.com/", (string) $request->getBody());
	}
	public function testCreatesHeadAndDeleteRequests() 
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("DELETE", "http://www.test.com/");
		$this->assertEquals("DELETE", $request->getMethod());
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("HEAD", "http://www.test.com/");
		$this->assertEquals("HEAD", $request->getMethod());
	}
	public function testCreatesOptionsRequests() 
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("OPTIONS", "http://www.example.com/");
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", "http://www.google.com/path?q=1&v=2", null, array( "a" => "b" ));
		$this->assertEquals(array( "a" => "b" ), $request->getPostFields()->getAll());
		unset($request);
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", "http://www.google.com/path?q=1&v=2", null, new \Guzzle\Common\Collection(array( "a" => "b" )));
		$this->assertEquals(array( "a" => "b" ), $request->getPostFields()->getAll());
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", "http://www.google.com/path?q=1&v=2", null, new \Guzzle\Http\QueryString(array( "a" => "b" )));
		$this->assertEquals(array( "a" => "b" ), $request->getPostFields()->getAll());
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", "http://www.test.com/", null, array( "a" => "b", "file" => "@" . __FILE__ ));
		$this->assertEquals(array( "a" => "b" ), $request->getPostFields()->getAll());
	...
	public function testCreatesFromParts() 
	{
		...
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->fromParts("PUT", $parts, null, "Data");
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("PUT", $request->getMethod());
		$this->assertEquals("http", $request->getScheme());
		...
		$this->assertEquals(array( "scheme" => "http", "host" => "www.google.com", "port" => 8080, "path" => "/path", "query" => "q=1&v=2" ), parse_url($request->getUrl()));
	}
	public function testCreatesFromMessage() 
	{
		$auth = base64_encode("michael:123");
		$message = "PUT /path?q=1&v=2 HTTP/1.1\r\nHost: www.google.com:8080\r\nContent-Length: 4\r\nAuthorization: Basic " . $auth . "\r\n\r\nData";
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->fromMessage($message);
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("PUT", $request->getMethod());
		$this->assertEquals("http", $request->getScheme());
		$this->assertEquals("http://www.google.com:8080/path?q=1&v=2", $request->getUrl());
		...
		$this->assertEquals("8080", $request->getPort());
		$this->assertFalse($request = \Guzzle\Http\Message\RequestFactory::getInstance()->fromMessage(""));
		$message = "PUT /path?q=1&v=2 HTTP/1.1\r\nHost: www.google.com\r\nContent-Length: 4\r\nAuthorization: Basic " . $auth . "\r\n\r\nData";
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->fromMessage($message);
		$this->assertInstanceOf("Guzzle\\Http\\Message\\EntityEnclosingRequest", $request);
		$this->assertEquals("PUT", $request->getMethod());
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("TRACE", "http://www.google.com/");
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/", array( "Transfer-Encoding" => "chunked" ), "hello");
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.foo.com/", array( "Transfer-Encoding" => "chunked" ), "Test");
		$this->assertFalse($request->hasHeader("Content-Length"));
		$this->assertEquals("chunked", $request->getHeader("Transfer-Encoding"));
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("POST", "http://www.foo.com/", array( "transfer-encoding" => "chunked" ), array( "foo" => "bar" ));
		$this->assertFalse($request->hasHeader("Content-Length"));
		$this->assertEquals("chunked", $request->getHeader("Transfer-Encoding"));
	}
	public function testClonesRequestsWithMethodWithoutClient() 
	{
		$f = \Guzzle\Http\Message\RequestFactory::getInstance();
	...
	{
		$this->getServer()->enqueue(array( "HTTP/1.1 307\r\nLocation: " . $this->getServer()->getUrl() . "\r\nContent-Length: 0\r\n\r\n" ));
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://foo.com", array( ), null, array( "query" => array( "Foo" => "Bar" ) ));
		$this->assertEquals("Bar", $request->getQuery()->get("Foo"));
	}
	public function testCanSetDefaultQueryString() 
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.foo.com?test=abc");
		\Guzzle\Http\Message\RequestFactory::getInstance()->applyOptions($request, array( "query" => array( "test" => "123", "other" => "t123" ) ), \Guzzle\Http\Message\RequestFactory::OPTIONS_AS_DEFAULTS);
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://foo.com", array( ), null, array( "auth" => array( "michael", "test" ) ));
		$this->assertEquals("michael", $request->getUsername());
		$this->assertEquals("test", $request->getPassword());
	}
	public function testCanAddDigestAuth() 
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://foo.com", array( ), null, array( "auth" => array( "michael", "test", "digest" ) ));
	...
	public function testCanAddEvents() 
	{
		$foo = null;
		$client = new \Guzzle\Http\Client();
		$client->addSubscriber(new \Guzzle\Plugin\Mock\MockPlugin(array( new \Guzzle\Http\Message\Response(200) )));
		$request = $client->get($this->getServer()->getUrl(), array( ), array( "events" => array( "request.before_send" => function() use (&$foo) 
		{
			$foo = true;
		}
		) ));
	...
	public function testCanAddEventsWithPriority() 
	{
		...
		$client = new \Guzzle\Http\Client();
		$client->addSubscriber(new \Guzzle\Plugin\Mock\MockPlugin(array( new \Guzzle\Http\Message\Response(200) )));
		$request = $client->get($this->getServer()->getUrl(), array( ), array( "events" => array( "request.before_send" => array( function() use (&$foo) 
		{
			$foo = true;
		}
		, 100 ) ) ));
	...
	{
		$mock = new \Guzzle\Plugin\Mock\MockPlugin(array( new \Guzzle\Http\Message\Response(200), new \Guzzle\Http\Message\Response(200) ));
		$client = new \Guzzle\Http\Client();
		$client->addSubscriber($mock);
		$request = $client->get("/", array( ), array( "plugins" => array( $mock ) ));
		$request->send();
	}
	public function testCanDisableExceptions() 
	{
		$client = new \Guzzle\Http\Client();
		$request = $client->get("/", array( ), array( "plugins" => array( new \Guzzle\Plugin\Mock\MockPlugin(array( new \Guzzle\Http\Message\Response(500) )) ), "exceptions" => false ));
	...
	{
		$r = \Guzzle\Http\EntityBody::factory();
		$client = new \Guzzle\Http\Client();
		$request = $client->get("/", array( ), array( "plugins" => array( new \Guzzle\Plugin\Mock\MockPlugin(array( new \Guzzle\Http\Message\Response(200, array( ), "testing") )) ), "save_to" => $r ));
		$request->send();
	...
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.foo.com", array( "Foo" => "Bar" ));
		\Guzzle\Http\Message\RequestFactory::getInstance()->applyOptions($request, array( "headers" => array( "Foo" => "Baz", "Bam" => "t123" ) ), \Guzzle\Http\Message\RequestFactory::OPTIONS_AS_DEFAULTS);
		$this->assertEquals("Bar", (string) $request->getHeader("Foo"));
	...
	public function inputValidation() 
	{
		...
		, array( "headers", "query", "cookies", "auth", "events", "plugins", "params" ));
	}
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://test.com", array( ), "0");
