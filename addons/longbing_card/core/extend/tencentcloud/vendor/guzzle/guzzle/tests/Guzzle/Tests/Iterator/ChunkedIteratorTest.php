...
class ChunkedIteratorTest extends \PHPUnit_Framework_TestCase 
{
	...
	{
		$chunked = new \Guzzle\Iterator\ChunkedIterator(new \ArrayIterator(range(0, 100)), 10);
	...
	{
		$chunked = new \Guzzle\Iterator\ChunkedIterator(new \ArrayIterator(array( 1, 2, 3, 4, 5 )), 2);
		$chunks = iterator_to_array($chunked, false);
	...
	public function testMustNotTerminateWithTraversable() 
	{
		...
		$chunked = new \Guzzle\Iterator\ChunkedIterator($traversable, 2);
		$actual = iterator_to_array($chunked, false);
		$this->assertCount(2, $actual);
	}
	public function testSizeOfZeroMakesIteratorInvalid() 
	{
		$chunked = new \Guzzle\Iterator\ChunkedIterator(new \ArrayIterator(range(1, 5)), 0);
		$chunked->rewind();
		$this->assertFalse($chunked->valid());
	}
	public function testSizeLowerZeroThrowsException() 
	{
		new \Guzzle\Iterator\ChunkedIterator(new \ArrayIterator(range(1, 5)), -1);
	}
}
?>