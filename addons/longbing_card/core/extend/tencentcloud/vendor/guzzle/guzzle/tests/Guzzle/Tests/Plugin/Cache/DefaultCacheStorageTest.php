...
class DefaultCacheStorageTest extends \Guzzle\Tests\GuzzleTestCase 
{
	protected function getCache() 
	{
		...
		$c = new \Guzzle\Cache\DoctrineCacheAdapter($a);
		$s = new \Guzzle\Plugin\Cache\DefaultCacheStorage($c);
		$request = new \Guzzle\Http\Message\Request("GET", "http://foo.com", array( "Accept" => "application/json" ));
		$response = new \Guzzle\Http\Message\Response(200, array( "Content-Type" => "application/json", "Connection" => "close", "X-Foo" => "Bar", "Vary" => "Accept" ), "test");
	...
	{
		$cache = $this->getCache();
		$this->assertNull($cache["storage"]->fetch(new \Guzzle\Http\Message\Request("GET", "http://test.com")));
	}
	...
	public function testPurgeRemovesAllMethodCaches() 
	{
		...
		{
			$request = \Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($cache["request"], $method);
			$cache["storage"]->cache($request, $cache["response"]);
		}
		$cache["storage"]->purge("http://foo.com");
		$this->assertFalse(in_array("test", $this->readAttribute($cache["cache"], "data")));
		$this->assertFalse(in_array($cache["serialized"], $this->readAttribute($cache["cache"], "data")));
		$this->assertEquals(array( "DoctrineNamespaceCacheKey[]" ), array_keys($this->readAttribute($cache["cache"], "data")));
	}
	public function testRemovesExpiredResponses() 
	{
		$cache = $this->getCache();
		$request = new \Guzzle\Http\Message\Request("GET", "http://xyz.com");
		$response = new \Guzzle\Http\Message\Response(200, array( "Age" => 1000, "Cache-Control" => "max-age=-10000" ));
	...
	public function testUsesVaryToDetermineResult() 
	{
		...
		$request = new \Guzzle\Http\Message\Request("GET", "http://foo.com", array( "Accept" => "application/xml" ));
		$this->assertNull($cache["storage"]->fetch($request));
	}
	...
	{
		return array( array( new \Guzzle\Http\Message\Request("GET", "http://foo.com", array( "Accept" => "foo" )), new \Guzzle\Http\Message\Response(200, array( "Cache-Control" => "stale-if-error=100", "Vary" => "Accept" )) ), array( new \Guzzle\Http\Message\Request("GET", "http://foo.com", array( "Accept" => "foo" )), new \Guzzle\Http\Message\Response(200, array( "Cache-Control" => "stale-if-error", "Vary" => "Accept" )) ) );
	}
