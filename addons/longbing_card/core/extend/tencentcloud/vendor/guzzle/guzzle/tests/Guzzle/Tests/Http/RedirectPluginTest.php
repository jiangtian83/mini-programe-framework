...
class RedirectPluginTest extends \Guzzle\Tests\GuzzleTestCase 
{
	public function testRedirectsRequests() 
	{
		...
		$this->getServer()->enqueue(array( "HTTP/1.1 301 Moved Permanently\r\nLocation: /redirect1\r\nContent-Length: 0\r\n\r\n", "HTTP/1.1 301 Moved Permanently\r\nLocation: /redirect2\r\nContent-Length: 0\r\n\r\n", "HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n" ));
		...
		$this->assertEquals(2, $request->getParams()->get(\Guzzle\Http\RedirectPlugin::REDIRECT_COUNT));
	...
	public function testCanLimitNumberOfRedirects() 
	{
		...
		catch( \Guzzle\Http\Exception\TooManyRedirectsException $e ) 
		{
	...
	public function testCanRedirectWithStrictRfcCompliance() 
	{
		...
		$request->getParams()->set(\Guzzle\Http\RedirectPlugin::STRICT_REDIRECTS, true);
		$request->send();
		$requests = $this->getServer()->getReceivedRequests(true);
		$this->assertEquals("POST", $requests[0]->getMethod());
	...
	{
		$this->getServer()->flush();
		$this->getServer()->enqueue(array( "HTTP/1.1 303 Moved Permanently\r\nLocation: /redirect\r\nContent-Length: 0\r\n\r\n", "HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n" ));
		$client = new \Guzzle\Http\Client($this->getServer()->getUrl());
