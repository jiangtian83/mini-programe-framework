...
class StreamTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$stream = new \Guzzle\Stream\Stream(true);
	}
	public function testConstructor() 
	{
		$handle = fopen("php://temp", "r+");
		fwrite($handle, "data");
		$stream = new \Guzzle\Stream\Stream($handle);
	...
	public function testCanModifyStream() 
	{
		...
		$stream = new \Guzzle\Stream\Stream($handle1);
	...
	public function testEnsuresSizeIsConsistent() 
	{
		$h = fopen("php://temp", "r+");
		fwrite($h, "foo");
		$stream = new \Guzzle\Stream\Stream($h);
	...
	{
		$stream = new \Guzzle\Stream\Stream(fopen("php://temp", "w+"));
		$stream->write("foobazbar");
	...
	{
		$r = fopen("php://temp", "w+");
		$stream = new \Guzzle\Stream\Stream($r);
		$stream->detachStream();
