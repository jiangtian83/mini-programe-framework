...
class CompositeFactoryTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		return $mock = $this->getMockBuilder($class)->disableOriginalConstructor()->getMock();
	}
	public function testIsIterable() 
	{
		$factory = new \Guzzle\Service\Command\Factory\CompositeFactory(array( $this->getFactory(), $this->getFactory() ));
	...
	public function testFindsFactories() 
	{
		...
		$factory = new \Guzzle\Service\Command\Factory\CompositeFactory(array( $f1, $f2 ));
	...
	{
		$factory = new \Guzzle\Service\Command\Factory\CompositeFactory();
		...
		$factory = new \Guzzle\Service\Command\Factory\CompositeFactory(array( $f1 ));
	...
	public function testAllowsRemovalOfFactories() 
	{
		...
		$factory = new \Guzzle\Service\Command\Factory\CompositeFactory($factories);
		$factory->remove("foo");
	...
	public function testProvidesDefaultChainForClients() 
	{
		...
		$chain = \Guzzle\Service\Command\Factory\CompositeFactory::getDefaultChain($client);
