...
class CurlMultiTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		parent::setUp();
		$this->multi = new \Guzzle\Tests\Mock\MockMulti();
	}
	...
	public function testRequestsCanBeAddedAndCounted() 
	{
		...
		$request1 = new \Guzzle\Http\Message\Request("GET", "http://www.google.com/");
		$multi->add($request1);
		$this->assertEquals(array( $request1 ), $multi->all());
		$request2 = new \Guzzle\Http\Message\Request("POST", "http://www.google.com/");
		$multi->add($request2);
	...
	public function testRequestsCanBeRemoved() 
	{
		...
		$request2 = new \Guzzle\Http\Message\Request("PUT", "http://www.google.com/");
	...
	{
		$this->multi->add(new \Guzzle\Http\Message\Request("GET", "http://www.google.com/"));
		$this->multi->reset();
		$this->assertEquals(array( ), $this->multi->all());
	}
	public function testSendsRequestsThroughCurl() 
	{
		$this->getServer()->enqueue(array( "HTTP/1.1 204 No content\r\n" . "Content-Length: 0\r\n" . "Server: Jetty(6.1.3)\r\n\r\n", "HTTP/1.1 200 OK\r\n" . "Content-Type: text/html; charset=utf-8\r\n" . "Content-Length: 4\r\n" . "Server: Jetty(6.1.3)\r\n\r\n" . "data" ));
		$request1 = new \Guzzle\Http\Message\Request("GET", $this->getServer()->getUrl());
		$request2 = new \Guzzle\Http\Message\Request("GET", $this->getServer()->getUrl());
		$this->multi->add($request1);
		$this->multi->add($request2);
		$this->multi->send();
	...
	public function testSendsThroughCurlAndAggregatesRequestExceptions() 
	{
		...
		$request1 = new \Guzzle\Http\Message\Request("GET", $this->getServer()->getUrl());
		$request2 = new \Guzzle\Http\Message\Request("HEAD", $this->getServer()->getUrl());
		$request3 = new \Guzzle\Http\Message\Request("GET", $this->getServer()->getUrl());
		$this->multi->add($request1);
		$this->multi->add($request2);
	...
	public function testCurlErrorsAreCaught() 
	{
		...
		try 
		{
			$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://127.0.0.1:9876/");
	...
	{
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", "http://127.0.0.1:9876/");
		$r = new \Guzzle\Http\Message\Response(200);
		$request->setClient(new \Guzzle\Http\Client());
	...
	public function testRemovesQueuedRequestsAddedInTransit() 
	{
		...
		$r = $client->get();
		$r->getEventDispatcher()->addListener("request.receive.status_line", function(\Guzzle\Common\Event $event) use ($client) 
		{
			$request = $client->get()->setResponse(new \Guzzle\Http\Message\Response(200), true);
			$request->send();
		}
		);
		$r->send();
	...
	public function testCatchesExceptionsWhenRemovingQueuedRequests() 
	{
		...
		{
			$client->get()->setResponse(new \Guzzle\Http\Message\Response(404), true)->send();
		}
		);
		try 
		{
	...
	public function testDoesNotCatchRandomExceptionsThrownDuringPerform() 
	{
		...
		$multi->expects($this->once())->method("perform")->will($this->throwException(new \RuntimeException("test")));
	...
	public function testDoesNotSendRequestsDecliningToBeSent() 
	{
		...
		$client = new \Guzzle\Http\Client("http://127.0.0.1:123", array( "curl.CURLOPT_PORT" => 123, "curl.CURLOPT_CONNECTTIMEOUT_MS" => 1 ));
		$request = $client->get();
		$multi = new \Guzzle\Http\Curl\CurlMulti();
		$multi->add($request);
		$request->getEventDispatcher()->addListener("request.exception", function(\Guzzle\Common\Event $event) use ($multi) 
		{
	...
	public function testDoesNotThrowExceptionsWhenRequestsRecoverWithRetry() 
	{
		...
		$client = new \Guzzle\Http\Client($this->getServer()->getUrl());
		$request = $client->get();
		$request->getEventDispatcher()->addListener("request.before_send", function(\Guzzle\Common\Event $event) 
		{
			$event["request"]->setResponse(new \Guzzle\Http\Message\Response(200));
		}
		);
		$multi = new \Guzzle\Http\Curl\CurlMulti();
		$multi->add($request);
	...
	public function testDoesNotThrowExceptionsWhenRequestsRecoverWithSuccess() 
	{
		...
		$request->getEventDispatcher()->addListener("request.exception", function(\Guzzle\Common\Event $event) use (&$count) 
		{
			$event["request"]->setResponse(new \Guzzle\Http\Message\Response(200));
		}
		);
	...
	{
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n");
		$request = new \Guzzle\Http\Message\Request("PUT", $this->getServer()->getUrl());
		$that = $this;
