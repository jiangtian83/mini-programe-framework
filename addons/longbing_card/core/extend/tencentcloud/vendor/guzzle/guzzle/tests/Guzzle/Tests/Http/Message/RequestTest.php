...
class RequestTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$this->client = new \Guzzle\Http\Client($this->getServer()->getUrl());
	...
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.guzzle-project.com/", array( "foo" => "bar" ));
	...
	{
		$this->assertInternalType("array", \Guzzle\Http\Message\Request::getAllEvents());
	}
	public function testConstructorBuildsRequestWithCollectionHeaders() 
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.guzzle-project.com/", new \Guzzle\Common\Collection(array( "foo" => "bar" )));
		$this->assertEquals("bar", $request->getHeader("foo"));
	}
	public function testConstructorBuildsRequestWithNoHeaders() 
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.guzzle-project.com/", null);
		$this->assertFalse($request->hasHeader("foo"));
	}
	public function testConstructorHandlesNonBasicAuth() 
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.guzzle-project.com/", array( "Authorization" => "Foo bar" ));
	...
	public function testRequestsCanBeConvertedToRawMessageStrings() 
	{
		...
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", "http://www.google.com/path?q=1&v=2", array( "Authorization" => "Basic " . $auth ), "Data");
	...
	public function testRequestStringsIncludeAuth() 
	{
		...
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("PUT", $this->getServer()->getUrl(), null, "Data")->setClient($this->client)->setAuth("michael", "123", CURLAUTH_BASIC);
		$request->send();
	...
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://test.com");
	...
	{
		$response = new \Guzzle\Http\Message\Response(200, array( "Content-Length" => 3 ), "abc");
	...
	public function testRequestThrowsExceptionOnBadResponse() 
	{
		try 
		{
			$this->request->setResponse(new \Guzzle\Http\Message\Response(404, array( "Content-Length" => 3 ), "abc"), true);
			$this->request->send();
	...
	{
		$this->assertEquals(\Guzzle\Http\Message\RequestInterface::STATE_NEW, $this->request->getState());
		$this->request->setState(\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER);
		$this->assertEquals(\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER, $this->request->getState());
	}
	public function testSetManualResponse() 
	{
		$response = new \Guzzle\Http\Message\Response(200, array( "Date" => "Sat, 16 Oct 2010 17:27:14 GMT", "Expires" => "-1", "Cache-Control" => "private, max-age=0", "Content-Type" => "text/html; charset=ISO-8859-1" ), "response body");
	...
	public function testRequestCanHaveManuallySetResponseBody() 
	{
		...
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 4\r\n\r\ndata");
		$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", $this->getServer()->getUrl());
		$request->setClient($this->client);
		$entityBody = \Guzzle\Http\EntityBody::factory(fopen($file, "w+"));
	...
	{
		$this->request->setState(\Guzzle\Http\Message\RequestInterface::STATE_COMPLETE);
	}
	public function testClonedRequestsUseNewInternalState() 
	{
		...
		$this->assertEquals(\Guzzle\Http\Message\RequestInterface::STATE_NEW, $r->getState());
	...
	public function testRequestCanBeSentUsingCurl() 
	{
		...
		try 
		{
			$request = \Guzzle\Http\Message\RequestFactory::getInstance()->create("GET", $this->getServer()->getUrl() . "index.html");
			$request->setClient($this->client);
			$response = $request->send();
	...
	{
		$this->getServer()->flush();
		$this->getServer()->enqueue(array( "HTTP/1.1 404 NOT FOUND\r\n" . "Content-Length: 0\r\n" . "\r\n", "HTTP/1.1 200 OK\r\n" . "Content-Length: 0\r\n" . "\r\n" ));
		$newResponse = null;
		$request = $this->request;
	...
	{
		$request = new \Guzzle\Http\Message\Request("GET", "http://www.example.com/foo?abc=d");
	...
	{
		$this->getServer()->flush();
		$this->getServer()->enqueue(array( "HTTP/1.1 303 SEE OTHER\r\nContent-Length: 0\r\n\r\n", "HTTP/1.1 301 Foo\r\nLocation: /foo\r\nContent-Length: 0\r\n\r\n" ));
		$request = $this->request;
	...
	{
		$en = \Guzzle\Http\EntityBody::factory();
		$request = $this->client->get();
		$request->setResponseBody($en);
		$request->setResponse(new \Guzzle\Http\Message\Response(200, array( ), "foo"));
