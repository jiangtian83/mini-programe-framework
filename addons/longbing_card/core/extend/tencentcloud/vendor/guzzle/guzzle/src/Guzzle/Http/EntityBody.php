...
class EntityBody extends \Guzzle\Stream\Stream implements EntityBodyInterface 
{
	...
	public static function factory($resource = "", $size = NULL) 
	{
		...
		{
			case "string": return self::fromString($resource);
			case "resource": return new static($resource, $size);
			case "object": if( method_exists($resource, "__toString") ) 
			{
				return self::fromString((string) $resource);
			}
			break;
			case "array": return self::fromString(http_build_query($resource));
		}
		throw new \Guzzle\Common\Exception\InvalidArgumentException("Invalid resource type");
	}
	public function setRewindFunction($callable) 
	{
		if( !is_callable($callable) ) 
		{
			throw new \Guzzle\Common\Exception\InvalidArgumentException("Must specify a callable");
		}
	...
	public function uncompress($filter = "zlib.inflate") 
	{
		...
		if( $filter == "zlib.inflate" ) 
		{
			...
			if( stream_get_contents($this->stream, 3, 0) === "\x1F媆x08" ) 
			{
				$offsetStart = 10;
			}
		}
	...
	{
		\Guzzle\Common\Version::warn("Guzzle\\Http\\EntityBody" . " is deprecated. Use getContentMd5()");
