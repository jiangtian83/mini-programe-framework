...
class UrlTest extends \Guzzle\Tests\GuzzleTestCase 
{
	...
	{
		$url = \Guzzle\Http\Url::factory("");
		$this->assertEquals("", (string) $url);
	}
	public function testPortIsDeterminedFromScheme() 
	{
		$this->assertEquals(80, \Guzzle\Http\Url::factory("http://www.test.com/")->getPort());
		$this->assertEquals(443, \Guzzle\Http\Url::factory("https://www.test.com/")->getPort());
		$this->assertEquals(null, \Guzzle\Http\Url::factory("ftp://www.test.com/")->getPort());
		$this->assertEquals(8192, \Guzzle\Http\Url::factory("http://www.test.com:8192/")->getPort());
	}
	public function testCloneCreatesNewInternalObjects() 
	{
		$u1 = \Guzzle\Http\Url::factory("http://www.test.com/");
		$u2 = clone $u1;
		$this->assertNotSame($u1->getQuery(), $u2->getQuery());
	}
	public function testValidatesUrlPartsInFactory() 
	{
		$url = \Guzzle\Http\Url::factory("/index.php");
		$this->assertEquals("/index.php", (string) $url);
		$this->assertFalse($url->isAbsolute());
		$url = "http://michael:test@test.com:80/path/123?q=abc#test";
		$u = \Guzzle\Http\Url::factory($url);
	...
	{
		$url = \Guzzle\Http\Url::factory("http://0:50/0?0#0");
		...
		$url = \Guzzle\Http\Url::factory("0");
		$this->assertSame("0", (string) $url);
	}
	public function testBuildsRelativeUrlsWithFalsyParts() 
	{
		$url = \Guzzle\Http\Url::buildUrl(array( "host" => "0", "path" => "0" ));
		$this->assertSame("//0/0", $url);
		$url = \Guzzle\Http\Url::buildUrl(array( "path" => "0" ));
		$this->assertSame("0", $url);
	}
	public function testUrlStoresParts() 
	{
		$url = \Guzzle\Http\Url::factory("http://test:pass@www.test.com:8081/path/path2/?a=1&b=2#fragment");
		...
		$this->assertEquals(array( "fragment" => "fragment", "host" => "www.test.com", "pass" => "pass", "path" => "/path/path2/", "port" => 8081, "query" => "a=1&b=2", "scheme" => "http", "user" => "test" ), $url->getParts());
	}
	public function testHandlesPathsCorrectly() 
	{
		$url = \Guzzle\Http\Url::factory("http://www.test.com");
		...
		$parts["path"] = "";
		$this->assertEquals("http://www.test.com", \Guzzle\Http\Url::buildUrl($parts));
		$parts["path"] = "test";
		$this->assertEquals("http://www.test.com/test", \Guzzle\Http\Url::buildUrl($parts));
	}
	public function testAddsQueryStringIfPresent() 
	{
		$this->assertEquals("?foo=bar", \Guzzle\Http\Url::buildUrl(array( "query" => "foo=bar" )));
	}
	public function testAddsToPath() 
	{
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath(false));
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath(null));
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath(array( )));
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath(new \stdClass()));
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath(""));
		$this->assertEquals("http://e.com/base?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath("/"));
		$this->assertEquals("http://e.com/baz/foo", (string) \Guzzle\Http\Url::factory("http://e.com/baz/")->addPath("foo"));
		$this->assertEquals("http://e.com/base/relative?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath("relative"));
		$this->assertEquals("http://e.com/base/relative?a=1", (string) \Guzzle\Http\Url::factory("http://e.com/base?a=1")->addPath("/relative"));
		$this->assertEquals("http://e.com/base/0", (string) \Guzzle\Http\Url::factory("http://e.com/base")->addPath("0"));
		$this->assertEquals("http://e.com/base/0/1", (string) \Guzzle\Http\Url::factory("http://e.com/base")->addPath("0")->addPath("1"));
	}
	...
	{
		$this->assertEquals($c, (string) \Guzzle\Http\Url::factory($a)->combine($b));
	}
	public function testHasGettersAndSetters() 
	{
		...
		$this->assertEquals("b=boo", (string) $url->setQuery(new \Guzzle\Http\QueryString(array( "b" => "boo" )))->getQuery());
	...
	{
		return array( array( "/foo/..", "/" ), array( "//foo//..", "/" ), array( "/foo/../..", "/" ), array( "/foo/../.", "/" ), array( "/./foo/..", "/" ), array( "/./foo", "/foo" ), array( "/./foo/", "/foo/" ), array( "/./foo/bar/baz/pho/../..", "/foo/bar" ), array( "*", "*" ), array( "/foo", "/foo" ), array( "/abc/123/../foo/", "/abc/foo/" ), array( "/a/b/c/./../../g", "/a/g" ), array( "/b/c/./../../g", "/g" ), array( "/b/c/./../../g", "/g" ), array( "/c/./../../g", "/g" ), array( "/./../../g", "/g" ) );
	}
	public function testNormalizesPaths($path, $result) 
	{
		$url = \Guzzle\Http\Url::factory("http://www.example.com/");
	...
	{
		$a = \Guzzle\Http\Url::factory("http://a/b/c/d;p?q");
		$b = \Guzzle\Http\Url::factory($relative);
