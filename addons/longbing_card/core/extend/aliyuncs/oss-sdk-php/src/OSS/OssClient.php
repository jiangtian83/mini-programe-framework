...
class OssClient 
{
	public static $OSS_ACL_TYPES = NULL;
	...
	public function putBucketLiveChannel($bucket, $channelName, $channelConfig, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\PutLiveChannelResult($response);
		$info = $result->getData();
	...
	public function getLiveChannelInfo($bucket, $channelName, $options = NULL) 
	{
		...
		$options[self::OSS_SUB_RESOURCE] = "live";
		$response = $this->auth($options);
		$result = new Result\GetLiveChannelInfoResult($response);
		return $result->getData();
	}
	public function getLiveChannelStatus($bucket, $channelName, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\GetLiveChannelStatusResult($response);
		return $result->getData();
	}
	public function getLiveChannelHistory($bucket, $channelName, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\GetLiveChannelHistoryResult($response);
		return $result->getData();
	}
	public function listBucketLiveChannels($bucket, $options = NULL) 
	{
		...
		$options[self::OSS_OBJECT] = "/";
		$options[self::OSS_SUB_RESOURCE] = "live";
		$options[self::OSS_QUERY_STRING] = array( "prefix" => (isset($options["prefix"]) ? $options["prefix"] : ""), "marker" => (isset($options["marker"]) ? $options["marker"] : ""), "max-keys" => (isset($options["max-keys"]) ? $options["max-keys"] : "") );
		$response = $this->auth($options);
		$result = new Result\ListLiveChannelResult($response);
		$list = $result->getData();
	...
	public function putObject($bucket, $object, $content, $options = NULL) 
	{
		...
		{
			$result = new Result\CallbackResult($response);
		}
		else 
		{
			$result = new Result\PutSetDeleteResult($response);
	...
	public function appendObject($bucket, $object, $content, $position, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\AppendResult($response);
		return $result->getData();
	}
	...
	public function copyObject($fromBucket, $fromObject, $toBucket, $toObject, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\CopyObjectResult($response);
		return $result->getData();
	}
	...
	public function deleteObjects($bucket, $objects, $options = NULL) 
	{
		...
		$response = $this->auth($options);
		$result = new Result\DeleteObjectsResult($response);
		return $result->getData();
	}
	...
	public function generateMultiuploadParts($file_size, $partSize = 5242880) 
	{
		...
		$values = array( );
		for( $partSize = $this->computePartSize($partSize);
		0 < $size_count;
		$i++ ) 
		{
			$size_count -= $partSize;
	...
	private function getValue($options, $key, $default = NULL, $isCheckEmpty = false, $isCheckBool = false) 
	{
		...
		if( $isCheckBool && $value !== true && $value !== false ) 
		{
			$value = false;
		}
		return $value;
	...
	private function auth($options) 
	{
		...
		if( (int) $request->get_response_code() === 500 && $this->redirects <= $this->maxRetries ) 
		{
	...
	{
		if( isset($options[self::OSS_HEADERS][self::OSS_ACL]) && !empty($options[self::OSS_HEADERS][self::OSS_ACL]) && !in_array(strtolower($options[self::OSS_HEADERS][self::OSS_ACL]), self::$OSS_ACL_TYPES) ) 
		{
	...
	{
		$resource_uri = "";
		if( isset($options[self::OSS_BUCKET]) && "" !== $options[self::OSS_BUCKET] && $this->hostType === self::OSS_HOST_TYPE_IP ) 
		{
	...
	private function generateSignableQueryStringParam($options) 
	{
		...
		$signableList = array( self::OSS_PART_NUM, "response-content-type", "response-content-language", "response-cache-control", "response-content-encoding", "response-expires", "response-content-disposition", self::OSS_UPLOAD_ID, self::OSS_COMP, self::OSS_LIVE_CHANNEL_STATUS, self::OSS_LIVE_CHANNEL_START_TIME, self::OSS_LIVE_CHANNEL_END_TIME, self::OSS_PROCESS, self::OSS_POSITION );
	...
	private function generateSignableResource($options) 
	{
		...
		if( isset($options[self::OSS_BUCKET]) && "" !== $options[self::OSS_BUCKET] ) 
		{
			...
			if( $options[self::OSS_OBJECT] == "/" && $this->hostType !== self::OSS_HOST_TYPE_IP ) 
			{
				$signableResource .= "/";
			}
		}
