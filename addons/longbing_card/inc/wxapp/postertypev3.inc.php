...
if( $type ) 
{
	$list_img = pdo_getslice("longbing_card_poster", array( "uniacid" => $uniacid, "status" => 1, "type_id" => $type ), $limit, $count, array( "id", "title", "img", "top", "type_id", "cover" ), "", array( "top desc", "id desc" ));
}
else 
{
	if( $type == 0 ) 
	{
		$list_img = pdo_getslice("longbing_card_poster", array( "uniacid" => $uniacid, "status" => 1 ), $limit, $count, array( "id", "title", "img", "top", "type_id", "cover" ), "", array( "top desc", "id desc" ));
	}
}
foreach( $list_img as $index => $item ) 
{
	$list_img[$index]["img_2"] = $this->transImage($item["img"]);
	$list_img[$index]["img_2"] = str_replace("ttp://", "ttps://", $list_img[$index]["img_2"]);
	if( !strstr($list_img[$index]["img_2"], "ttps://") ) 
	{
		$list_img[$index]["img_2"] = "https://" . $list_img[$index]["img_2"];
	}
	$list_img[$index]["img"] = tomedia($item["img"]);
	if( $item["cover"] ) 
	{
		$list_img[$index]["cover"] = tomedia($item["cover"]);
	}
	else 
	{
		$list_img[$index]["cover"] = $list_img[$index]["img"];
	}
}
$data["post_img"] = $list_img;
$data["page"] = $curr;
$data["total_page"] = ceil($count / 15);
if( $user_info ) 
{
	...
	if( $user ) 
	{
		...
		$user["avatar_2"] = str_replace("ttp://", "ttps://", $user["avatar_2"]);
		if( !strstr($user["avatar_2"], "ttps://") ) 
		{
			$user["avatar_2"] = "https://" . $user["avatar_2"];
		}
		...
		$user["qr_path"] = str_replace("ttp://", "ttps://", $user["qr_path"]);
		if( !strstr($user["qr_path"], "ttps://") ) 
		{
			$user["qr_path"] = "https://" . $user["qr_path"];
		}
