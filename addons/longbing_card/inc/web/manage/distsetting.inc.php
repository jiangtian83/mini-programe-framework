...
{
	$data = array( );
	$data["first_extract"] = $_GPC["first_extract"];
	$data["cash_mini"] = $_GPC["cash_mini"];
	$data["admin_account"] = $_GPC["admin_account"];
	pdo_update("longbing_card_config", $data, array( "uniacid" => $uniacid ));
	if( $result === false ) 
	{
		message("编辑失败", "", "error");
	}
	message("编辑成功", $this->createWebUrl("manage/distSetting"), "success");
}
...
$id = $info["id"];
load()->func("tpl");
include($this->template("manage/distSetting"));
?>