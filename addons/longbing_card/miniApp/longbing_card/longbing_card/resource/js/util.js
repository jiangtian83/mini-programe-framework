...
util.getNewUrl = function (url) {
	...
	url = app.siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&from=wxapp&' + 'c=entry&a=wxapp&m=' + nowPage.__route__.split('/')[0] + '&do=' + url;
	// console.log(url, "url")
	return url;
}
...
util.request = function (option) {
	...
	option.hideLoading = typeof option.hideLoading != 'undefined' ? option.hideLoading : true;
	...
	if (!option.data || !option.data.m) {
		var nowPage = getCurrentPages();
		// console.log(nowPage,"ddddddddddddddddddddddd///////nowPage")
	...
	if (!userid) {
		...
		var utilParamUrl = app.siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&from=wxapp&' + 'c=entry&a=wxapp&do=';

		var loginUrl = utilParamUrl + 'login&m=' + nowPage.__route__.split('/')[0];
		...
		wx.login({
			success: function (res) {
				...
				if (fromID.length > 0) {
					loginParam.from_id = fromID
				}

				let loginParam = {
					code: res.code,
					scene: app.globalData.loginParam.scene,
					is_qr: app.globalData.loginParam.is_qr,
					is_group: app.globalData.loginParam.is_group,
					type: app.globalData.loginParam.type,
					target_id: app.globalData.loginParam.target_id,
				...
				wx.request({
					...
					'success': function (response) {
						...
						if (!response.data.errno) {
							...
							data.user_id = response.data.data.user_id;
							wx.setStorageSync('userid', response.data.data.user_id)
							app.globalData.userid = response.data.data.user_id;
							console.log(app.globalData.userid,"dddddddddddddddd  userid")
							url = url;
		...
		let data = option.data;
		data.user_id = userid;
		data.scene = app.globalData.loginParam.scene;
...
util.showLoading = function (type) {
	var tmpTitle;
	if (type == 1) {
		tmpTitle = '加载中'
	}
...
/**
 * 获取的URL地址   By.jingshuixian
 * 参考 : https://www.cnblogs.com/cench/p/6718466.html
 */
...
util.goUrl = function (e, isFrom = false) {

	let { url, method } = isFrom ? util.getFromData(e) : util.getData(e);
