...
class QRinput 
{
	...
	public static function lengthOfCode($mode, $version, $bits) 
	{
		...
		switch( $mode ) 
		{
			case QR_MODE_NUM: $chunks = (int) ($payload / 10);
			...
			break;
			case QR_MODE_8: $size = (int) ($payload / 8);
			break;
			case QR_MODE_KANJI: $size = (int) ($payload / 13 * 2);
			break;
			case QR_MODE_STRUCTURE: $size = (int) ($payload / 8);
			break;
			default: $size = 0;
			break;
		}
...
class QRsplit 
{
	...
	public function eatNum() 
	{
		...
		for( $p = 0; self::isdigitat($this->dataStr, $p);
		$p++ ) 
		{
		}
		$run = $p;
	...
	public function eatAn() 
	{
		...
		while( self::isalnumat($this->dataStr, $p) ) 
		{
			...
			{
				for( $q = $p; self::isdigitat($this->dataStr, $q);
				$q++ ) 
				{
				}
	...
	public function eat8() 
	{
		...
		while( $p < $dataStrLen ) 
		{
			...
			else 
			{
				if( $mode == QR_MODE_AN ) 
				{
					for( $q = $p; self::isalnumat($this->dataStr, $q);
					$q++ ) 
					{
					}
	...
	public function splitString() 
	{
		while( 0 < strlen($this->dataStr) ) 
		{
			...
			switch( $mode ) 
			{
				...
				break;
				case QR_MODE_KANJI: if( $hint == QR_MODE_KANJI ) 
				{
...
class QRrawcode 
{
	...
	public function init(array $spec) 
	{
		...
		$dataPos = 0;
		$eccPos = 0;
		for( $i = 0; $i < QRspec::rsBlockNum1($spec);
		$i++ ) 
		{
		...
		for( $i = 0; $i < QRspec::rsBlockNum2($spec);
		$i++ ) 
		{
			$ecc = array_slice($this->ecccode, $eccPos);
			$this->rsblocks[$blockNo] = new QRrsblock($dl, array_slice($this->datacode, $dataPos), $el, $ecc, $rs);
...
class QRencode 
{
	...
	public static function factory($level = QR_ECLEVEL_L, $size = 3, $margin = 4) 
	{
		...
		switch( $level . "" ) 
		{
			case "0": case "1": case "2": case "3": $enc->level = $level;
			break;
			case "l": case "L": $enc->level = QR_ECLEVEL_L;
			break;
			case "m": case "M": $enc->level = QR_ECLEVEL_M;
			break;
			case "q": case "Q": $enc->level = QR_ECLEVEL_Q;
			break;
			case "h": case "H": $enc->level = QR_ECLEVEL_H;
			break;
		}
		return $enc;
	}
