	...
	class QRcode 
	{
		...
		protected function calcN1N3($length) 
		{
			...
			for( $i = 0; $i < $length; $i++ ) 
			{
				...
				if( $i & 1 && 3 <= $i && $i < $length - 2 && $this->runLength[$i] % 3 == 0 ) 
				{
		...
		protected function identifyMode($pos) 
		{
			...
			if( $this->hint == QR_MODE_KJ && $pos + 1 < strlen($this->dataStr) ) 
			{
		...
		protected function eatNum() 
		{
			...
			for( $p = 0; $this->isdigitat($this->dataStr, $p);
			$p++ ) 
			{
			}
			$run = $p;
		...
		protected function eatAn() 
		{
			...
			while( $this->isalnumat($this->dataStr, $p) ) 
			{
				...
				{
					for( $q = $p; $this->isdigitat($this->dataStr, $q);
					$q++ ) 
					{
					}
		...
		protected function eat8() 
		{
			...
			while( $p < $dataStrLen ) 
			{
				...
				else 
				{
					if( $mode == QR_MODE_AN ) 
					{
						for( $q = $p; $this->isalnumat($this->dataStr, $q);
						$q++ ) 
						{
						}
		...
		protected function splitString() 
		{
			while( 0 < strlen($this->dataStr) ) 
			{
				...
				switch( $mode ) 
				{
					case QR_MODE_NM: $length = $this->eatNum();
					break;
					case QR_MODE_AN: $length = $this->eatAn();
					break;
					case QR_MODE_KJ: if( $hint == QR_MODE_KJ ) 
					{
		...
		protected function encodeBitStream($inputitem, $version) 
		{
			...
			else 
			{
				...
				{
					case QR_MODE_NM: $inputitem = $this->encodeModeNum($inputitem, $version);
					break;
					case QR_MODE_AN: $inputitem = $this->encodeModeAn($inputitem, $version);
					break;
					case QR_MODE_8B: $inputitem = $this->encodeMode8($inputitem, $version);
					break;
					case QR_MODE_KJ: $inputitem = $this->encodeModeKanji($inputitem, $version);
					break;
					case QR_MODE_ST: $inputitem = $this->encodeModeStructure($inputitem);
					break;
					default: break;
				}
			}
		...
		protected function estimateBitStreamSize($items, $version) 
		{
			...
			foreach( $items as $item ) 
			{
				switch( $item["mode"] ) 
				{
					case QR_MODE_NM: $bits = $this->estimateBitsModeNum($item["size"]);
					break;
					case QR_MODE_AN: $bits = $this->estimateBitsModeAn($item["size"]);
					break;
					case QR_MODE_8B: $bits = $this->estimateBitsMode8($item["size"]);
					break;
					case QR_MODE_KJ: $bits = $this->estimateBitsModeKanji($item["size"]);
					break;
		...
		protected function lengthOfCode($mode, $version, $bits) 
		{
			...
			switch( $mode ) 
			{
				case QR_MODE_NM: $chunks = (int) ($payload / 10);
				...
				break;
				case QR_MODE_AN: $chunks = (int) ($payload / 11);
				...
				break;
				case QR_MODE_8B: $size = (int) ($payload / 8);
				break;
				case QR_MODE_KJ: $size = (int) ($payload / 13 * 2);
				break;
				case QR_MODE_ST: $size = (int) ($payload / 8);
				break;
				default: $size = 0;
				break;
			}
